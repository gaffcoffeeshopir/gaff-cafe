...
function initSqlite() {
  ...
  ensureSqliteColumn("home_images", "sort_order", "INTEGER NOT NULL DEFAULT 0");
...
async function initPostgres(databaseUrl) {
  ...
  await pgPool.query(`ALTER TABLE home_images ADD COLUMN IF NOT EXISTS sort_order INTEGER NOT NULL DEFAULT 0`);
...
const HOME_HERO_SLOT = "hero";
const HOME_VIDEO_SLOT = "video";

function isGallerySlot(slot) {
  return /^gallery-[a-z0-9]+$/i.test(String(slot || "").trim());
}

function isValidHomeSlot(slot) {
  const s = String(slot || "").trim();
  return s === HOME_HERO_SLOT || s === HOME_VIDEO_SLOT || isGallerySlot(s);
}

function newGallerySlot() {
  return (
    "gallery-" +
...
function mapHomeImageRow(row) {
  const updatedAt = row.updated_at instanceof Date ? row.updated_at.toISOString() : String(row.updated_at || "");
  const file = String(row.file || "");
  const bust = updatedAt ? "?v=" + encodeURIComponent(updatedAt) : "";
  const slot = String(row.slot || "");
  let kind = "gallery";
  if (slot === HOME_HERO_SLOT) kind = "hero";
  else if (slot === HOME_VIDEO_SLOT) kind = "video";
  return {
    slot,
    kind,
    file,
    path: file ? "img/home/" + file + bust : "",
    mime: row.mime || "",
    caption: row.caption || "",
...
function partitionHomeImages(images) {
  const list = Array.isArray(images) ? images : [];
  const hero = list.find((img) => img.slot === HOME_HERO_SLOT) || null;
  const video = list.find((img) => img.slot === HOME_VIDEO_SLOT) || null;
  const gallery = list
    .filter((img) => img.slot !== HOME_HERO_SLOT && img.slot !== HOME_VIDEO_SLOT)
    .sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0) || String(a.slot).localeCompare(String(b.slot)));
  return { hero, video, gallery };
}

function listHomeImagesSqlite() {
  return sqlite
    .prepare(
      `SELECT slot, file, mime, caption, sort_order, updated_at
       FROM home_images
       ORDER BY CASE
         WHEN slot = 'hero' THEN 0
         WHEN slot = 'video' THEN 1
         ELSE 2
       END, sort_order ASC, updated_at ASC, slot ASC`
    )
    .all()
    .map(mapHomeImageRow);
}

async function listHomeImagesPostgres() {
...
function getHomeImageMetaSqlite(slot) {
  return (
    sqlite
      .prepare("SELECT slot, file, mime, caption, sort_order, updated_at FROM home_images WHERE slot = ?")
      .get(slot) || null
  );
}

async function getHomeImageMetaPostgres(slot) {
  const { rows } = await pgPool.query(
    "SELECT slot, file, mime, caption, sort_order, updated_at FROM home_images WHERE slot = $1",
    [slot]
  );
  return rows[0] || null;
}

function nextGallerySortSqlite() {
  const row = sqlite
    .prepare(
      "SELECT COALESCE(MAX(sort_order), 0) AS m FROM home_images WHERE slot NOT IN ('hero', 'video')"
    )
    .get();
  return (row && row.m ? Number(row.m) : 0) + 1;
}

async function nextGallerySortPostgres() {
  const { rows } = await pgPool.query(
    "SELECT COALESCE(MAX(sort_order), 0)::int AS m FROM home_images WHERE slot NOT IN ('hero', 'video')"
  );
  return (rows[0] && rows[0].m ? Number(rows[0].m) : 0) + 1;
}

function saveHomeImageSqlite(slot, file, mime, buffer, caption, sortOrder) {
  const prev = getHomeImageMetaSqlite(slot);
  let order = prev ? Number(prev.sort_order) || 0 : 0;
  if (!prev) {
    order =
      slot === HOME_HERO_SLOT || slot === HOME_VIDEO_SLOT
        ? 0
        : sortOrder != null
          ? Number(sortOrder)
          : nextGallerySortSqlite();
  }
  sqlite
    .prepare(
      `INSERT INTO home_images (slot, file, mime, data, caption, sort_order, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)
    ...
    .run(slot, file, mime, buffer, caption || null, order, new Date().toISOString());
  return prev ? prev.file : null;
}

async function saveHomeImagePostgres(slot, file, mime, buffer, caption, sortOrder) {
  const prev = await getHomeImageMetaPostgres(slot);
  let order = prev ? Number(prev.sort_order) || 0 : 0;
  if (!prev) {
    order =
      slot === HOME_HERO_SLOT || slot === HOME_VIDEO_SLOT
  ...
  await pgPool.query(
    `INSERT INTO home_images (slot, file, mime, data, caption, sort_order, updated_at)
     ...
     ON CONFLICT (slot) DO UPDATE SET
       ...
       caption = EXCLUDED.caption,
       updated_at = NOW()`,
    [slot, file, mime, buffer, caption || null, order]
  );
  return prev ? prev.file : null;
}

function deleteHomeImageSqlite(slot) {
  const prev = getHomeImageMetaSqlite(slot);
  if (!prev) return null;
  sqlite.prepare("DELETE FROM home_images WHERE slot = ?").run(slot);
  return prev.file;
}

async function deleteHomeImagePostgres(slot) {
  const prev = await getHomeImageMetaPostgres(slot);
  if (!prev) return null;
  await pgPool.query("DELETE FROM home_images WHERE slot = $1", [slot]);
  return prev.file;
}

function restoreHomeImagesToDiskSqlite(homeDir) {
  fs.mkdirSync(homeDir, { recursive: true });
  const rows = sqlite.prepare("SELECT file, data FROM home_images").all();
  for (const row of rows) {
    fs.writeFileSync(path.join(homeDir, row.file), row.data);
  }
  return rows.length;
}

async function restoreHomeImagesToDiskPostgres(homeDir) {
  fs.mkdirSync(homeDir, { recursive: true });
  const { rows } = await pgPool.query("SELECT file, data FROM home_images");
  for (const row of rows) {
    fs.writeFileSync(path.join(homeDir, row.file), row.data);
  }
  return rows.length;
...
async function listHomeImages() {
  if (mode === "postgres") return listHomeImagesPostgres();
  return listHomeImagesSqlite();
}

async function saveHomeImage(slot, file, mime, buffer, caption, sortOrder) {
  if (mode === "postgres") return saveHomeImagePostgres(slot, file, mime, buffer, caption, sortOrder);
  return saveHomeImageSqlite(slot, file, mime, buffer, caption, sortOrder);
}

async function deleteHomeImage(slot) {
  if (mode === "postgres") return deleteHomeImagePostgres(slot);
  return deleteHomeImageSqlite(slot);
}

async function restoreHomeImagesToDisk(homeDir) {
  if (mode === "postgres") return restoreHomeImagesToDiskPostgres(homeDir);
  return restoreHomeImagesToDiskSqlite(homeDir);
}
