...
function isValidHomeSlot(slot) {
  ...
  return s === HOME_HERO_SLOT || isGallerySlot(s);
}
...
function partitionHomeImages(images) {
  ...
  const gallery = list
    .filter((img) => img.slot !== HOME_HERO_SLOT)
  ...
  return sqlite
    .prepare(
      `SELECT slot, file, mime, caption, sort_order, updated_at
       FROM home_images
       ORDER BY CASE WHEN slot = 'hero' THEN 0 ELSE 1 END, sort_order ASC, updated_at ASC, slot ASC`
    )
    .all()
    .map(mapHomeImageRow);
}
...
function nextGallerySortSqlite() {
  const row = sqlite
    .prepare("SELECT COALESCE(MAX(sort_order), 0) AS m FROM home_images WHERE slot != 'hero'")
    .get();
...
async function nextGallerySortPostgres() {
  const { rows } = await pgPool.query(
    "SELECT COALESCE(MAX(sort_order), 0)::int AS m FROM home_images WHERE slot != 'hero'"
  );
...
function saveHomeImageSqlite(slot, file, mime, buffer, caption, sortOrder) {
  ...
  if (!prev) {
    order = slot === HOME_HERO_SLOT ? 0 : sortOrder != null ? Number(sortOrder) : nextGallerySortSqlite();
  }
  sqlite
    .prepare(
